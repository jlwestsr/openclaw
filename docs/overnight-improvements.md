...
**Problem:** When sessions are compacted (context window overflow), the merge summary instructions were generic: _"Preserve decisions, TODOs, open questions, and any constraints."_ This led to summaries that lost concrete details — file paths, error messages, specific tool outcomes, and user preferences were frequently dropped.
...
**Problem:** When tool results exceed the context window budget, they're truncated by keeping only the beginning. But errors, stack traces, and result summaries typically appear at the _end_ of tool output. A 400KB exec result with an error on the last line would lose the most important part.
